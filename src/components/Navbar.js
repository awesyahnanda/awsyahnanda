...
export default function Navbar({ darkMode, handleClick, active, setActive }) {
    ...
    return (
        <Box component={'nav'} position={singlePage ? 'fixed' : 'relative'} className={darkMode ? Style.dark : Style.light}>
            <Box component={'ul'} display={'flex'} justifyContent={'center'} alignItems={'center'}
                gap={{ xs: '2rem', md: '5.5rem' }}
                textTransform={'normal'} fontSize={'1.2rem'}>
